...
class Portfolio:
    def __init__(self, lots):
        lots = pd.DataFrame(lots).assign(
            timestamp=lambda df: pd.to_datetime(df["timestamp"])
        )
        lots["cost"] = (
            lots["cost"]
            .where(lots["cost"].notna(), lots["price"] * lots["quantity"])
            .astype({"cost": float})
        )
        self.lots = lots
    ...
    def status(self, anchor=None):
        ...
        latest = (
            performance.groupby(["timestamp_buy", "symbol"])
            .apply(
    ...
    def performance(self, indexes=None, anchor=None):
        ...
        history = pd.concat(
            [
                yf.stock(symbol, range="10y", interval="1d").dropna()
                for symbol in self.lots["symbol"].unique()
            ],
            sort=False,
        )
        ...
        dfh = history
        dfo = self.lots.assign(
            timestamp=lambda df: df["timestamp"].dt.tz_localize(current_tz)
        )

        performance = (
            dfo.merge(
                dfh[["symbol", "timestamp", "adjclose"]].rename(
                    {"adjclose": "price"}, axis=1
                ),
                on="symbol",
                suffixes=("_buy", ""),
            )
        ...
        performance = performance[
            performance["timestamp_buy"] <= performance["timestamp"]
        ]
